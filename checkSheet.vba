...
def parse_attr_type(buf: str):
    ...
    match buf:
        case "半角": return AttributeType.Narrow, ""
        case "全角": return AttributeType.Wide, ""
        case "英数字": return AttributeType.Alphanumeric, ""
        case "半角カナ": return AttributeType.NarrowKana, ""
        case "整数": return AttributeType.IntegerNumber, ""
        case "小数": return AttributeType.SmallNumber, ""
        case _ if buf.startswith("日付:"):
            fmt = buf.split(":", 1)[1]
            return AttributeType.Date, fmt
        case _:
            raise ValueError(f"Unknown attribute type: '{buf}'")


def parse_byte_edit_mode(attr: AttributeInfoItem, buf: str):
    if buf == "固定":
